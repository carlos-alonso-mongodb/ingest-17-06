...
import os, json, logging, threading, certifi, traceback, time, copy
...
def apply_shortcuts_if_enabled(doc: dict) -> dict:
    """
    Return a deep copy of *doc* with shortcuts applied
    – but only when APPLY_SHORTCUTS is True.
    """
    if not APPLY_SHORTCUTS:
        return doc
    return apply_sc(copy.deepcopy(doc)) 
...
def sc_key(key: str) -> str:                    
    return SHORTCUT_KEYS.get(key, key)          
...
def walk(node: dict,
    ...
    """Flatten *node* into the cn[ ] array."""

    # ── 0.  numeric code of *this* node ─────────────────────────────
    aid  = archetype_id(node)
    if aid:
        code = at_code_to_int(aid) if aid.lower().startswith("at") \
               else alloc_code("ar_code", aid)
    else:
    ...
    for k, v in node.items():
        ...
        if isinstance(v, list) and any(is_locatable(x) and
                                       split_me_as_a_new_node(x) for x in v):
            continue

        # 1e – ordinary scalar
        scalars[k] = v

    # ── 2.  date coercion + shortcuts ──────────────────────────────

    
    scalars                = to_bson_dates(scalars)   
    scalars["archetype_node_id"] = code     
    # ── 2b.  shrink archetype_details.ai --------------------------------
    ...
    if isinstance(ad, dict):
        ...
        if isinstance(ai_obj, dict):
            ...
            if isinstance(sid, str):
                num = alloc_code("ar_code", sid)      # look-up / allocate
                if num is not None:
...
def transform(raw: dict) -> tuple[dict, dict] | None:
    ...
    # ── 1.  flatten into cn[ ] ─────────────────────────────────────
    cn: List[dict] = []
    walk(comp, ancestors=(), cn=cn, kp_chain=[], list_index=None)

    # secondary ingestors must skip bad/unknown codes
    if _local.bad_code and cfg_global.get("role") != "primary":
        return None

    # ── 2.  build sn[ ]  (mapping rules) ──────────────────────────
    rules = get_rules_for(template_id, root_aid)
    sn: List[dict] = []

    for node in cn:
        # ---------- NEW BLOCK: build the set of path prefixes -----
        # node["p"] is leaf-to-root, e.g.  "-14.-7.3.-4.11"
        parts   = node["p"].split(".")
        # every **leaf-first prefix**: "-14", "-14.-7", "-14.-7.3", ...
        ...
        for rule in rules:
            rc  = rule["_path"]      # leaf-first pathChain (tuple)
            rc2 = rule["_cont"]      # root-first contains   (tuple)

            # pathChain  (leaf-first)
            if rc and ".".join(str(x) for x in rc) not in pc_set:
                continue

            # contains  (root-first)
            if rc2:
                j = 0
                for code in reversed(parts[1:]):   # walk ancestors root-first
                    if code == str(rc2[j]):
                        j += 1
                        if j == len(rc2):
                            break
            ...
            # extra dotted predicates
            if any(dpath_get({"data": dblock}, path) != val_req
                   for path, val_req in rule["_extra"]):
                continue

            # — copy the requested fields into a slim block —
            slim: dict = {}
    ...
    # ── 3.  ancestry marker never reaches Mongo ───────────────────
    for node in cn:
        node.pop("_anc", None)
    ...
    base_doc   = apply_shortcuts_if_enabled(base_doc)
    search_doc = apply_shortcuts_if_enabled(search_doc)
...
def init_worker(cfg: dict, ca: str) -> None:
    ...
    """
    global SRC, TGT, SEARCH, alloc_code, APPLY_SHORTCUTS
    ...
    APPLY_SHORTCUTS = cfg.get("apply_shortcuts", True)
    ...
    if APPLY_SHORTCUTS:
        ...
        logging.info("🗂  Shortcut tables loaded (%d keys, %d values)",
                     len(SHORTCUT_KEYS), len(SHORTCUT_VALS))
    else:
        logging.info("🗂  Shortcut tables disabled by configuration")
